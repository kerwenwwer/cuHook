...
#define CU_HOOK_PRINT(format, ...) fprintf(stderr, "CUDA Hook Library: " format, ##__VA_ARGS__)
...
static void* real_dlsym(void *handle, const char* symbol)
{
    ...
    if (!internal_dlsym) {
        #ifdef __x86_64__
            internal_dlsym = (fnDlsym)dlvsym(RTLD_NEXT, "dlsym", "GLIBC_2.2.5");
        #elif defined(__i386__)
            internal_dlsym = (fnDlsym)dlvsym(RTLD_NEXT, "dlsym", "GLIBC_2.0");
        #else
            #error "Unsupported architecture"
        #endif

        if (!internal_dlsym) {
            fprintf(stderr, "Error getting dlsym: %s\n", dlerror());
            exit(1);
        }
    }
